...
router.post('/add_employee', upload.single('image'), (req, res) => {
    ...
    bcrypt.hash(req.body.password, 10, (err, hash) => {
        ...
        const values = [
            ...
            req.body.address,
            req.body.salary,
            image, // req.body.image yerine dosya adını koyuyoruz
            req.body.category_id
        ];

        console.log("Values to insert:", values); // eklenen veriyi kontrol et

        con.query(sql, values, (err, result) => {
            if (err) {
                console.error("MySQL Insert Error:", err); // terminalde tüm hatayı göster
                return res.json({ Status: false, Error: err.sqlMessage }); // tarayıcıya gerçek MySQL hatasını döndür
            }
            return res.json({ Status: true });
        });
    });
...
router.put('/edit_employee/:id', (req, res) => {
  ...
  // zorla integer yap (boş stringleri engelle)
  const catId = category_id ? parseInt(category_id, 10) : null;
  if (!catId) return res.json({ Status: false, Error: "category_id is required" });

  // category var mı kontrol et
  const checkSql = "SELECT id FROM category WHERE id = ?";
  con.query(checkSql, [catId], (err, catResult) => {
    if (err) return res.json({ Status: false, Error: "Query error: " + err });
    ...
    // Güncelleme
    const sql = 'UPDATE employee SET name = ?, email = ?, salary = ?, address = ?, category_id = ? WHERE id = ?';
    con.query(sql, [name, email, salary, address, catId, id], (err, result) => {
      if (err) return res.json({ Status: false, Error: "Query error: " + err });
      return res.json({ Status: true, Result: result });
    });
  });
